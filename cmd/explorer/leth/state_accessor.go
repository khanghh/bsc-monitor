...
func (leth *LightEthereum) StateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool, preferDisk bool) (statedb *state.StateDB, err error) {
	var (
		current  *types.Block
	...
	if checkLive {
		statedb, err = leth.blockchain.StateAt(block.Root())
		if err == nil {
			return statedb, nil
		}
	}
	if base != nil {
		if preferDisk {
			...
			database = state.NewDatabaseWithConfig(leth.chainDb, &trie.Config{Cache: 512})
		...
		current = leth.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if current == nil {
		...
		for i := uint64(0); i < reexec; i++ {
			...
			parent := leth.blockchain.GetBlock(current.ParentHash(), current.NumberU64()-1)
			if parent == nil {
	...
	for current.NumberU64() < origin {
		...
		if current = leth.blockchain.GetBlockByNumber(next); current == nil {
			return nil, fmt.Errorf("block #%d not found", next)
		}
		statedb.SetExpectedStateRoot(current.Root())
		statedb, _, _, _, err := leth.blockchain.Processor().Process(current, statedb, vm.Config{})
		if err != nil {
			return nil, fmt.Errorf("processing block %d failed: %v", current.NumberU64(), err)
		}
		// Finalize the state so any modifications are written to the trie
		statedb.Finalise(leth.blockchain.Config().IsEIP158(current.Number()))
...
func (leth *LightEthereum) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	// Short circuit if it's genesis block.
	if block.NumberU64() == 0 {
		return nil, vm.BlockContext{}, nil, errors.New("no transaction in genesis")
	}
	// Create the parent state database
	parent := leth.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.BlockContext{}, nil, fmt.Errorf("parent %#x not found", block.ParentHash())
	}
	// Lookup the statedb of parent block from the live database,
	// otherwise regenerate it on the flight.
	statedb, err := leth.StateAtBlock(parent, reexec, nil, true, false)
	if err != nil {
	...
	for idx, tx := range block.Transactions() {
		...
		if posa, ok := leth.Engine().(consensus.PoSA); ok && msg.From() == context.Coinbase &&
