...
func inspectExtDB(ctx *cli.Context) error {
	...
	cfg := makeAppConfig(ctx)
	stack := newServiceStack(&cfg.Service)
	defer stack.Stop()

	db, err := stack.OpenDatabase(indexerDataDir, cfg.Indexer.DatabaseCache, cfg.Indexer.DatabaseHandles, indexerDbNamespace, false)
	if err != nil {
