...
// ReplayProcessor is a basic Processor, which takes care of transitioning
// state from one point to another.
//
// ReplayProcessor implements Processor.
...
// NewReplayProcessor initialises a new StateProcessor.
func NewReplayProcessor(config *params.ChainConfig, bc *core.BlockChain, engine consensus.Engine) *ReplayProcessor {
...
func (p *ReplayProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (*state.StateDB, types.Receipts, []*types.Log, uint64, error) {
	var (
		usedGas     = new(uint64)
		header      = block.Header()
	...
	bloomProcessors := core.NewAsyncReceiptBloomGenerator(txNum)
